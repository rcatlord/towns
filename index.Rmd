...
output$plot <- renderPlot({
    ggplot(indicator(), aes(x = indicator, y = value, colour = value)) +
      ...
      scale_color_viridis() +
      scale_y_continuous(expand = c(0, 0), limits = c(0, NA), labels = percent_format(accuracy = 1)) +
...
output$summary <- renderUI({
    ...
    HTML(paste0("<p><span class='town_name'>", town_id, "</span> is a ", tolower(filter(summary_stats, town_name == town_id)$Size)," town in the local authority district of ", filter(summary_stats, town_name == town_id)$area_name, ". In mid-2019 it had an estimated population of ", comma(filter(summary_stats, town_name == town_id)$`Total population`), " residents. ", percent(filter(summary_stats, town_name == town_id)$`Aged 0 to 15`), " of residents were aged under 15, ", percent(filter(summary_stats, town_name == town_id)$`Aged 16 to 64`), " were of working age (16-64), and ", percent(filter(summary_stats, town_name == town_id)$`Aged 65+`), " were aged 65 years or over.</p>"))
  })
```